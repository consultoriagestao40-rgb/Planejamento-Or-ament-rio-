...
export default function BudgetEntryGrid({ costCenterId, year, taxRate = 0 }: BudgetEntryGridProps) {
    ...
    const treeRoots = useMemo(() => {
        ...
        const syntheticParents = [
            ...
            { code: '2.1.3', name: '2.1.3 - Retenção na fonte' },
        ...
        try {
            ...
            for (let i = 0; i < 12; i++) {
                ...
                if (valueChanged || obsChanged) {
                    ...
                    // Only for valid database categories
                    allIds.forEach((id: string) => {
                        if (id !== targetId && !id.startsWith('synth-')) {
                            const otherCat = categories.find((c: any) => c.id === id);
                            if (otherCat) {
                                entries.push({
                                    categoryId: id,
                                    month: i,
                                    year,
                                    costCenterId: costCenterId === 'Geral' ? 'DEFAULT' : costCenterId,
                                    tenantId: otherCat.tenantId || tenantId,
                                    amount: 0,
                                    observation: null,
                                    isLocked: false
                                });
