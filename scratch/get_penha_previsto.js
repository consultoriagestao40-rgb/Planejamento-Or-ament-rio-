...
async function main() {
    try {
        console.log('--- DISTRIBUIÇÃO MENSAL DE PREVISTO_RECEBER EM 2026 ---');
        
        const summary = await prisma.realizedEntry.groupBy({
            by: ['tenantId', 'month'],
            where: { year: 2026, viewMode: 'previsto_receber' },
            _count: true,
            _sum: { amount: true }
        });
        ...
        summary.sort((a,b) => a.month - b.month).forEach(s => {
            console.log(`Empresa: ${tenantMap[s.tenantId] || s.tenantId}`);
            console.log(`  - Mês:   ${s.month}`);
