...
export default function BudgetGrid({
    ...
    const handleExportPDF = () => {
        ...
        const companyNames = selectedCompany.includes('DEFAULT')
            ? 'Todas as Empresas'
            : selectedCompany.map(id => companies.find(c => c.id === id)?.name || id).join(', ');

        // Resolve cost center names
        const ccNames = selectedCostCenter.includes('DEFAULT')
            ? 'Todos os Centros de Custos'
            : selectedCostCenter.map(id => filteredCostCenters.find(cc => cc.id === id)?.name || id).join(', ');

        const startMonthName = MONTHS[startMonth];
        const endMonthName = MONTHS[endMonth];
        const periodStr = viewPeriod === 'month'
            ? `${startMonthName} a ${endMonthName} ${selectedYear}`
        ...
        printWindow.document.write(`
            ...
            <head>
                <meta charset="utf-8" />
                <title>Relatório DRE - ${companyNames}</title>
                <style>
            ...
            <body>
                <div class="report-header">
                    <div class="report-title-container">
                        <div>
                            <h1 class="report-title">Relatório de Planejamento Financeiro (DRE)</h1>
                ...
                <tr className={isInteractiveTree ? `spreadsheet-parent-row spreadsheet-parent-row-level-${node.level + 1}` : ''}>
                    <td 
                        ...
                        style={{ 
                            ...
                            paddingLeft: `${0.5 + (node.level * 1.2)}rem`,
                            borderBottom: '1px solid #e2e8f0',
                            transition: 'all 0.2s cubic-bezier(0.4, 0, 0.2, 1)',
                            width: '400px',
                            minWidth: '400px',
                            maxWidth: '400px',
                            fontSize: isInteractiveTree ? '0.8rem' : '0.74rem'
                        }}
                    >
                        <div style={{ display: 'flex', alignItems: 'center', color: '#0f172a' }}>
                            {isInteractiveTree && (
                                <span 
                                    className="bi-tree-btn"
                                    style={{ 
                                        color: hasCompositionChildren ? '#8b5cf6' : '#475569',
                                        borderColor: hasCompositionChildren ? '#cbd5e1' : '#94a3b8'
                                    }}
                                >
                                    {isExpanded ? '−' : '+'}
                                </span>
                            )}
                            {!isInteractiveTree && <span style={{ display: 'inline-block', width: '20px' }}></span>}
                            <span style={{ 
                                whiteSpace: 'normal',
                ...
                {isExpanded && node.children.length === 0 && (() => {
                    ...
                    return Array.from(allItemNames).sort().map(itemName => (
                        <tr key={`${node.id}-${itemName}`} style={{ background: 'rgba(241, 245, 249, 0.4)' }}>
                            <td className="sticky-col" style={{ paddingLeft: `${1.5 + (node.level * 1.2)}rem`, fontSize: '0.74rem', color: '#475569', borderBottom: '1px solid #e2e8f0', width: '400px', minWidth: '400px', maxWidth: '400px' }}>
                                <div style={{ display: 'flex', alignItems: 'center', width: '100%', overflow: 'hidden' }}>
                                    <span style={{ marginRight: '0.5rem', color: '#cbd5e1', flexShrink: 0 }}>└</span>
                                    <span style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontSize: '0.74rem' }} title={itemName}>
                                        {itemName}
                                    </span>
                                </div>
                            </td>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                        ...
                        <button
                            ...
                            onMouseOver={(e) => {
                                e.currentTarget.style.transform = 'translateY(-1px)';
                                e.currentTarget.style.boxShadow = '0 4px 8px rgba(225, 29, 72, 0.3)';
                            }}
                            onMouseOut={(e) => {
                                e.currentTarget.style.transform = 'none';
                                e.currentTarget.style.boxShadow = '0 2px 4px rgba(225, 29, 72, 0.2)';
                            }}
                        >
                            <span>📥</span>
